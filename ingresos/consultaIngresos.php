...
function colorEstatus($id)
	...
	<input type="text" name="IdEntPry" id="IdEntPry" value="'.$IdEntPry.'" style="visibility:hidden; display:none;"/>
    </tr>
			<tr align="center">
...
//echo '<input type="fecha" name="date'.$cont.'" id="date'.$cont.'" placeholder="Ingresa fecha" onchange="actualizarFecha('.$renglon['IdEntPag'].',this.value,'.$cont.');"/>';	
								...
								echo '<td id="fec'.$cont.'">'.date('d-m-Y',strtotime($renglon['FecEntPagRal'])).
								'</td><td><img src="../vista/images/config.gif" title="Modificar Fecha" onclick="verificaPass('.$renglon['IdEntPag'].','.$cont.','.$pagoi.');"  width="25" align="right"></td>';
								}
							echo '
							</tr>
							</table>
							...
							<td>Cliente</td><td><input type="button" onclick="programaDisp('.$cont.')" value="Programar"></td>
					...
					echo '<tr align="center" class="registro">
             		   ...
             		   <div id="dialogregistro" title="Registrar Pago">
					   <table align="center" border="1">
						...
						<div id="dialogmod" title="Modificar Fecha">
					   <table align="center" border="1">
							<tr align="center">
								<td>
									Fecha de Pago
						...
						<script>enviaTotal('.$totalpry.','.$cuenta.','.$cuentavalidar.','.round($totalporcral,3).','.$alerta.')</script>';
