	...
	public class Webcam extends Sprite {
		...
		public function init(){
			...
			if (camera != null) {
				...
				ExternalInterface.addCallback('_changeCam', changeCam);
		...
		public function configure(panel:String = SecurityPanel.PRIVACY) {
		...
		/**
		* Cambia de una camara a otra para tomar foto
		*
		*/
		public function changeCam(){
			if(Camera.names.length > 1){
				releaseCamera();
				cameraIdx = camera.index;
				if(cameraIdx + 1 > Camera.names.length - 1){
					cameraIdx = 0;
				}else{
